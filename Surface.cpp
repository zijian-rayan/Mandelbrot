...
{
  ASSERT(nBitDepth == 24 || nBitDepth == 16 || nBitDepth == 32); // no palettes !
  ...
  m_pSrcWnd = pWnd;
  ASSERT(m_hDestDC);
  VERIFY(m_image.Create(width, -m_wndHeight, m_nBitDepth, 0)); // pass negative Height to make it Top Down.
...
void CSurface::Create(CWnd *pWnd, int nBitDepth, int w, int h)
{
  ASSERT(nBitDepth == 24 || nBitDepth == 16 || nBitDepth == 32); // no palettes !
  Destroy();
  m_nBitDepth = nBitDepth;
  ...
  m_pSrcWnd = pWnd;
  ASSERT(m_hDestDC);
  VERIFY(m_image.Create(w, -h, m_nBitDepth, 0)); // pass negative Height to make it Top Down.
  ASSERT((DWORD_PTR(m_image.GetBits()) & 0xf) == 0); // make sure we are at least 128 bit aligned.(SSE2)
  ASSERT(m_image.GetPitch() > 0); // Verify top down DIB
  VERIFY(m_image.GetDC() != NULL); // Prefer the DC to exist for life of object
}
...
{
  image.BitBlt(m_image.GetDC(), 0, 0, GetVisibleWidth(), GetVisibleHeight(), 0, 0);
  m_image.ReleaseDC();
  BlitBits();
}
...
void CSurface::StripeBits()
{
  ...
  {
    COLORREF color = count++;
    for (int j = 0; j < m_wndWidth; j++) PointColor(j, i, color);
  }
}
...
void CSurface::Line(const CPoint &p1, const CPoint &p2, COLORREF c)
{
  ...
  int d, deltax, deltay, numpixels, dinc1, dinc2, xinc1, xinc2, yinc1, yinc2;
  ...
  for (int i = numpixels; i > 0; i--) {
    PointColor(x1, y1, c);
    if (d < 0) { d += dinc1; x1 += xinc1; y1 += yinc1; }
    else { d += dinc2; x1 += xinc2; y1 += yinc2; }
  }
}
